...
Elm.Main.make = function (_elm) {
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var splitEveryFromRight = F2(function (n,str) {
      var numSplits = $String.length(str) / n | 0;
      var _p0 = A3($List.foldl,
      F2(function (i,_p1) {
         var _p2 = _p1;
         var _p3 = _p2._0;
         return {ctor: "_Tuple2"
                ,_0: A2($String.dropLeft,n,_p3)
                ,_1: A2($List._op["::"],
                $String.reverse(A2($String.left,n,_p3)),
                _p2._1)};
      }),
      {ctor: "_Tuple2",_0: $String.reverse(str),_1: _U.list([])},
      _U.range(0,numSplits - 1));
      var rem = _p0._0;
      var splits = _p0._1;
      return $String.isEmpty(rem) ? splits : A2($List._op["::"],
      $String.reverse(rem),
      splits);
   });
   var withCommas = function (price) {
      var parts = A2($String.split,".",$Basics.toString(price));
      var rhs = A2($Maybe.withDefault,
      "",
      $List.head(A2($List.drop,1,parts)));
      var lhs = A2($Maybe.withDefault,"",$List.head(parts));
      var lhsCommas = A2($String.join,
      "",
      A2($List.intersperse,",",A2(splitEveryFromRight,3,lhs)));
      var $final = $String.isEmpty(rhs) ? lhsCommas : A2($Basics._op["++"],
      lhsCommas,
      A2($Basics._op["++"],".",rhs));
      return $final;
   };
   var displayPrice = function (price) {
      var str = withCommas($Basics.toFloat($Basics.truncate(price)));
      return A2($Basics._op["++"],"$",str);
   };
   var update = F2(function (action,model) {
      var _p4 = action;
      if (_p4.ctor === "UpdatePrice") {
            return _U.update(model,{price: _p4._0});
         } else {
            return _U.update(model,{rate: _p4._0});
         }
   });
   var UpdatePrice = function (a) {
      return {ctor: "UpdatePrice",_0: a};
   };
   var sendPriceUpdate = F2(function (address,str) {
      var cleanStr = A4($Regex.replace,
      $Regex.All,
      $Regex.regex("[$,]"),
      $Basics.always(""),
      str);
      var _p5 = $String.toFloat(cleanStr);
      if (_p5.ctor === "Ok") {
            return A2($Signal.message,address,UpdatePrice(_p5._0));
         } else {
            return A2($Signal.message,address,UpdatePrice(0));
         }
   });
   var UpdateRate = function (a) {
      return {ctor: "UpdateRate",_0: a};
   };
   var Model = F2(function (a,b) {    return {price: a,rate: b};});
   var rateToString = function (rate) {
      var _p6 = rate;
      switch (_p6.ctor)
   ...
   var rateChoiceClasses = F2(function (model,rate) {
      var selectedClass = _U.eq(model.rate,rate) ? "selected" : "";
      return $Html$Attributes.$class(A2($Basics._op["++"],
      "rate-choice ",
      A2($Basics._op["++"],
      rateToString(rate),
      A2($Basics._op["++"]," ",selectedClass))));
   });
   var Monthly = {ctor: "Monthly"};
   var initModel = {price: 50,rate: Monthly};
   var Weekly = {ctor: "Weekly"};
   var Once = {ctor: "Once"};
   var defaultRate = 1.07;
   var sumOfTrueCost = F2(function (yearlyPrice,years) {
      return A3($List.foldl,
      F2(function (y,sum) {
         return sum + yearlyPrice * Math.pow(defaultRate,
         $Basics.toFloat(years - y));
      }),
      0.0,
      _U.range(0,years - 1));
   });
   var calculateTrueCost = F2(function (model,years) {
      var _p7 = model.rate;
      switch (_p7.ctor)
      {case "Once": return model.price * Math.pow(1.07,
           $Basics.toFloat(years));
         case "Weekly": return A2(sumOfTrueCost,
           model.price * 52.5,
           years);
         default: return A2(sumOfTrueCost,model.price * 12,years);}
   });
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.h1,
              _U.list([]),
              _U.list([$Html.text("The true cost")]))
              ,A2($Html.div,
              _U.list([]),
              _U.list([$Html.text("of your purchases")]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("cost-container")]),
              _U.list([$Html.text("$")
                      ,A2($Html.input,
                      _U.list([A3($Html$Events.on,
                              "input",
                              $Html$Events.targetValue,
                              sendPriceUpdate(address))
                              ,$Html$Attributes.type$("text")
                              ,$Html$Attributes.value($Basics.toString(model.price))]),
                      _U.list([]))]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("rate-container")]),
              _U.list([A2($Html.span,
                      _U.list([A2(rateChoiceClasses,model,Once)
                              ,A2($Html$Events.onClick,address,UpdateRate(Once))]),
                      _U.list([$Html.text(rateToString(Once))]))
                      ,A2($Html.span,
                      _U.list([A2(rateChoiceClasses,model,Monthly)
                              ,A2($Html$Events.onClick,address,UpdateRate(Monthly))]),
                      _U.list([$Html.text(rateToString(Monthly))]))
                      ,A2($Html.span,
                      _U.list([A2(rateChoiceClasses,model,Weekly)
                              ,A2($Html$Events.onClick,address,UpdateRate(Weekly))]),
                      _U.list([$Html.text(rateToString(Weekly))]))]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("true-cost-container")]),
              _U.list([A2($Html.div,
                      _U.list([$Html$Attributes.$class("true-cost-dollars")]),
                      _U.list([$Html.text(displayPrice(A2(calculateTrueCost,
                      model,
                      10)))]))
                      ,A2($Html.div,
                      _U.list([$Html$Attributes.$class("true-cost-years")]),
                      _U.list([$Html.text("if invested for 10 years")]))]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("true-cost-container")]),
              _U.list([A2($Html.div,
                      _U.list([$Html$Attributes.$class("true-cost-dollars")]),
              ...
              _U.list([A2($Html.div,
                      ...
                      ,A2($Html.div,
                      _U.list([$Html$Attributes.$class("true-cost-years")]),
                      _U.list([$Html.text("30 years")]))]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("footer")]),
              _U.list([A2($Html.div,
                      _U.list([]),
                      _U.list([$Html.text("Assuming 7% compounded")]))
                      ,A2($Html.div,
                      _U.list([]),
                      _U.list([$Html.text("Inspired by ")
                              ,A2($Html.a,
                              _U.list([$Html$Attributes.href("http://www.mrmoneymustache.com/2011/04/15/getting-started-3-eliminate-short-termitis-the-bankruptcy-disease/")]),
                              _U.list([$Html.text("MMM")]))]))
                      ,A2($Html.div,
                      _U.list([]),
                      _U.list([A2($Html.a,
                      _U.list([$Html$Attributes.href("https://github.com/elben/true-cost")]),
                      _U.list([$Html.text("Source code")]))]))]))]));
   });
   var main = $StartApp$Simple.start({model: initModel
                                     ,update: update
                                     ,view: view});
   return _elm.Main.values = {_op: _op
                             ...
                             ,sumOfTrueCost: sumOfTrueCost
                             ,initModel: initModel
                             ,update: update
                             ,sendPriceUpdate: sendPriceUpdate
                             ,splitEveryFromRight: splitEveryFromRight
                             ,withCommas: withCommas
                             ,displayPrice: displayPrice
                             ,rateChoiceClasses: rateChoiceClasses
                             ,view: view};
};
